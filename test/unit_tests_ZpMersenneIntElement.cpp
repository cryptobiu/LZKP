...
TEST_CASE("cac_prover_logic<ZpMersenneIntElement>_constructor") {
  ...
  ZpMersenneIntElement **a;
  ZpMersenneIntElement *t, *secret;

  a = allocate2D<ZpMersenneIntElement>(n, m);
  t = allocate1D<ZpMersenneIntElement>(n);
  secret = allocate1D<ZpMersenneIntElement>(m);
...
TEST_CASE("cac_verifier_logic<ZpMersenneIntElement>_r8_reject") {
  auto M = 50, tau = 13, N = 4, n = 4, m = 32;
...
TEST_CASE("cac_verifier_logic<ZpMersenneIntElement>_protocol_big_numbers") {
...
TEST_CASE("cac_verifier_logic<ZpMersenneIntElement>_protocol_big_numbers_reject") {
  auto M = 200, tau = 100, N = 16, n = 128, m = 1024;
...
TEST_CASE("sac_prover_logic<ZpMersenneIntElement>_r3") {
  ...
  for (auto e = 0; e < M; ++e) {
    ...
    ZpMersenneIntElement v_sigma = ZpMersenneIntElement(0);
    for (auto i = 0; i < N; ++i) {
      o_sigma += p.provers_[e]->o_[i];
      v_sigma += p.provers_[e]->v_[i];
    }
    REQUIRE(o_sigma == ZpMersenneIntElement(0));
    REQUIRE(v_sigma == ZpMersenneIntElement(0));
  }
...
TEST_CASE("sac_verifier_logic<ZpMersenneIntElement>_r4") {
  auto M = 50, N = 8, n = 4, m = 32;
