...
ProverWrapper::ProverWrapper(const Settings &s, const NTL::Mat<NTL::ZZ_p> &a, const NTL::Vec<NTL::ZZ_p> &t, const NTL::Vec<NTL::ZZ_p> &secret)
...
void ProverWrapper::r1(block &h_gamma) {
...
void ProverWrapper::r3(const std::vector<bool> &E, std::vector<block> &seed, std::vector<block> &omegaN, block &h_pi) {
  E_ = E;
...
void ProverWrapper::r5(const block &seed_ell, block &h_psi) {
  seed_ell_ = seed_ell;
  ...
  for (auto e = 0; e < M; ++e) {
    ...
    for (auto i = 0; i < n; ++i) {
      provers_[e]->coefficients_[i] = NTL::ZZ_p(prng_seed_ell_.get<block>().halves[0]);
    }
    for (auto i = 0; i < m; ++i) {
      provers_[e]->coefficients_[n + i] = NTL::ZZ_p(prng_seed_ell_.get<block>().halves[0]);
    }
...
void ProverWrapper::r7(const std::vector<int> &i_bar, block &seed_e_bar, std::vector<std::vector<block>> &seed_tree,
